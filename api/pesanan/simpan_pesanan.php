...
try {
    ...
    if (isset($data['proses_pembayaran'])) {
        ...
        $stmt_pembayaran = $conn->prepare("INSERT INTO proses_pembayaran (kode_payment, id_pesanan, id_bayar, id_user, status, jumlah_uang, jumlah_dibayarkan, kembalian, id_tagihan, model_diskon, nilai_nominal, total_diskon) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt_pembayaran->bind_param("siiisdddssdd", $kode_payment, $id_pesanan_baru, $pembayaran['id_bayar'], $pembayaran['id_user'], $pembayaran['status'], $pembayaran['jumlah_uang'], $pembayaran['jumlah_dibayarkan'], $pembayaran['kembalian'], $id_tagihan, $pembayaran['model_diskon'], $pembayaran['nilai_nominal'], $pembayaran['total_diskon']);
